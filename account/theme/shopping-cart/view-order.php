...
$title = _('Shopping Cart - View Order') . ' | ' . TITLE;
...
css( "shopping-cart/view.css" );
javascript( "shopping-cart/view-order.js" );
...
<div id="content">
	...
	<div id="subcontent">
		...
		<p id="pShipping">
            ...
            <?php echo _('Main Phone:'); echo $order['billing_phone']; ?><br />
           	<?php if( isset( $order['billing_alt_phone'] ) ) { echo _('Alternate Phone:'); echo $order['billing_alt_phone']; } ?><br />
            <br />
            <?php } ?>
			...
			<select name="sStatus" id="sStatus">
				<option value="0"<?php if( '0' == $order['status'] ) echo ' selected="selected"'; ?>>Purchased</option>
				<option value="1"<?php if( '1' == $order['status'] ) echo ' selected="selected"'; ?>>Pending</option>
				<option value="2"<?php if( '2' == $order['status'] ) echo ' selected="selected"'; ?>>Delivered</option>
				<option value="-1"<?php if( '-1' == $order['status'] ) echo ' selected="selected"'; ?>>Declined</option>
			</select>
		...
		<table cellpadding="0" cellspacing="0" id="tPricing" width="500">
			<tr>
				<td width="20%" style="font-weight:bold;" valign="top">
					...
					. _('Tax:<br />')
					. _('Shipping:<br />');
					if( $order['website_coupon_id'] ) echo 'Coupon Discount:<br />';
					echo '<br />'
					. _('<strong>Total:</strong>');
                    ?>
				</td>
		...
		<table cellpadding="0" cellspacing="0" id="tItemList">
			...
			<!-- End: Table Heading -->
			<?php 
			foreach( $order['items'] as $website_order_item_id => $item ) {
			?>
			<tr>
				...
				<td class="description">
					...
					<?php 
					if( !$item['status'] )
						echo _('<p class="out-of-stock">(Out of Stock)</p>');
					?>
					...
					?>
						<a href="#" id="aExpandOptions<?php echo $website_order_item_id; ?>" class="expand-options" title="Show Options"><strong><span id="sExpandOptions<?php echo $website_order_item_id; ?>">[ + ]</span> Options</strong></a><br />
