...
$email_autoresponder_id = (int) $_GET['eaid'];
...
if( nonce::verify( $_POST['_nonce'], 'add-edit-autoresponder' ) ) {
	$errs = $v->validate();
		...
		<form name="fAddEditAutoresponder" action="/email-marketing/autoresponders/add-edit/?eaid=<?php echo $email_autoresponder_id; ?>" method="post">
			<table cellpadding="0" cellspacing="0">
				<tr>
					...
					<?php } else { ?>
					<td><input type="text" class="tb" name="tName" id="tName" maxlength="80" value="<?php echo ( !$success && !empty( $_POST['tName'] ) || empty( $autoresponder['name'] ) ) ? $_POST['tName'] : $autoresponder['name']; ?>" /></td>
					<?php } ?>
				</tr>
				<tr>
					<td><label for="tSubject"><?php echo _('Subject'); ?>:</label></td>
					<td><input type="text" class="tb" name="tSubject" id="tSubject" maxlength="80" value="<?php echo ( !$success && !empty( $_POST['tSubject'] ) || empty( $autoresponder['subject'] ) ) ? $_POST['tSubject'] : $autoresponder['subject']; ?>" /></td>
				</tr>
				<tr>
					...
					<td class="top">
						...
						if( 1 == $autoresponder['default'] ) {
							...
							$i = 0;
							$email_list_id = ( !$success && !empty( $_POST['rbEmailListID'] ) || empty( $autoresponder['email_list_id'] ) ) ? $_POST['rbEmailListID'] : $autoresponder['email_list_id'];
			...
			<br />
			<textarea name="taMessage" id="taMessage" cols="50" rows="5" rte="1"><?php echo ( !$success && !empty( $_POST['taMessage'] ) || empty( $autoresponder['message'] ) ) ? $_POST['taMessage'] : $autoresponder['message']; ?></textarea>
			<br />
			<p><input type="checkbox" class="cb" name="cbCurrentOffer" id="cbCurrentOffer" value="1"<?php if( !$success && '1' == $_POST['cbCurrentOffer'] || $autoresponder['current_offer'] ) echo ' checked="checked"'; ?> /> <label for="cbCurrentOffer"><?php echo _('Include Current Offer'); ?></label></p>
