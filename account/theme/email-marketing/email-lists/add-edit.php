...
if( nonce::verify( $_POST['_nonce'], 'add-edit-email-list' ) ) {
	$errs = $v->validate();
		...
		<form name="fAddEditEmailList" action="/email-marketing/email-lists/add-edit/<?php if ( $email_list_id ) echo "?elid=$email_list_id"; ?>" method="post">
			<table cellpadding="0" cellspacing="0">
				<tr>
					...
					<td><input type="text" class="tb" name="tName" id="tName" maxlength="80" value="<?php echo ( !$success && !empty( $_POST['tName'] ) || empty( $email_list['name'] ) ) ? $_POST['tName'] : $email_list['name']; ?>" /></td>
				</tr>
				<tr>
					<td valign="top"><label for="taDescription"><?php echo _('Description'); ?>:</label></td>
					<td><textarea name="taDescription" id="taDescription" cols="35" rows="6"><?php echo ( !$success && !empty( $_POST['taDescription'] ) || empty( $email_list['description'] ) ) ? $_POST['taDescription'] : $email_list['description']; ?></textarea></td>
				</tr>
