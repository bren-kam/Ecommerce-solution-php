...
class CompressCSS {
	...
	private function parse( $contents ) {
		...
		$css_array = $selector_array = array();
		...
		foreach ( $rules as $r ) {
			...
			while( $declaration_key < count( $declarations ) ) {
                ...
                $value = ( isset( $d_array[1] ) ) ? $d_array[1] : '';
		...
		foreach ( $css_array as $declaration => $selectors ) {
            ...
            if ( stristr( $property, 'background' ) && stristr( $value, 'gradient' ) && stristr( $value, '/' ) ) {
                // Handle special split
                $values = explode ( '/', $value );

                foreach ( $values as $v ) {
                    $new_declaration = $property . ':' . trim( $v );
	...
	private function minimize_properties( $declarations ) {
		...
		foreach ( $new_declarations as $property => $d ) {
            if ( stristr( $property, 'background' ) )
            foreach ( $d as $index => $declaration ) {
                if ( stristr( $declaration, 'gradient' ) ) {
