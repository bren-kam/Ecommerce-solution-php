...
$.extend(Boxy, {
    ...
    DEFAULTS: {
        ...
        closeText:              '<img src="/images/icons/x-grey.png" width="13" height="12" alt="Close" />',      // text to use for default close link
    ...
    ask: function(question, answers, callback, options) {
        ...
        buttons.html($.map(Boxy._values(answers), function(v) {
    ...
    _setupModalResizing: function() {
        if (!Boxy.resizeConfigured) {
            var w = $(window).resize(Boxy._handleResize);
    ...
    _cssForOverlay: function() {
        if (Boxy.IE6) {
            ...
            return {width: '100%', height: $(document).height()};
        }
    }
...
Boxy.prototype = {
    ...
    setContent: function(newContent) {
        newContent = $(newContent).css({display: 'block'}).addClass('boxy-content').find('.boxy-break').remove().end().append('<br class="boxy-break" clear="all" />');
    ...
    show: function() {
        ...
        if (this.options.modal) {
            ...
            this.modalBlackout = $('<div class="boxy-modal-blackout"></div>')
                .css($.extend(Boxy._cssForOverlay(), {
    ...
    _setupTitleBar: function() {
        if (this.options.title) {
            ...
            if (this.options.closeable) {
                tb.append($('<a href="javascript:;" class="close"></a>').html(this.options.closeText));
            }
