var MediaManager = {
    ...
    defaultView: {
        container:
            ...
            '<button type="button" class="btn btn-primary" id="mm-submit" data-dismiss="modal">Insert into Post</button>' +
            '</div>' +
            '</div>' +
            '</div>' +
            '</div>'
        ...
        , uploader:
            '<div class="row">'+
            '<div class="col-lg-9">'+
            '<input type="text" class="form-control" id="mm-file-name" placeholder="Please set a file name before selecting a file..." />' +
            '</div>' +
            '<div class="col-lg-3">'+
            '<button type"button" class="btn btn-default" id="mm-select-file" disabled>Select file</button>' +
            '</div>' +
            '</div>' +
    ...
    , uploaderComplete: function( id, filename, response ) {
        ...
        if ( response.success ) {
            ...
            MediaManager.appendFile( response );
        }
    }
    ...
    , appendFile: function( file ) {
        ...
        var fileTag = MediaManager.isImage( file ) ?
                        $( '<img />', { src: file.url } ) :
                        $( '<a />', { href: file.url }).text( file.name );
