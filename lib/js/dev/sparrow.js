...
var sparrow = function(context) {
	...
	if( tables.length )
	head.load( 'http://ajax.aspnetcdn.com/ajax/jquery.dataTables/1.9.4/jquery.dataTables.min.js', '/resources/js_single/?f=jquery.datatables', function() {
		// Make each table
	...
	if( dialogs.length )
	head.load( '/resources/js_single/?f=jquery.boxy', function() {
		// Make dialogs
	...
	if ( ajaxForms.length )
	head.load( '/resources/js_single/?f=jquery.form', function() {
...
function ajaxResponse( response ) {
	...
	if( response['success'] ) {
		...
		if( response['refresh'] ) {
			...
			head.load( '/resources/js_single/?f=jquery.php', function() {
				// Interpret jQuery
				php( response['jquery'] );
			});
		}
    ...
    if ( response['notification'] ) {
        var error = ( response['notification']['success'] ) ? '' : ' error';
        var notification = '<div class="notification sticky hidden' + error + '">';
        notification += '<a class="close" href="#"><img src="/images/icons/close.png" alt="Close" /></a>';
        notification += '<p>' + response['notification']['message'] + '</p>';
        notification += '</div>';
        $('body').append(notification);
        $('.notification:last').notify();
    }
}