...
class FoursquareApi {
	 ...
	 * @param array $params 
	 */
	public function FILE($url,$params=[]){
		$get = [];
		$get['oauth_token'] = $this->AuthToken;
		$get['v'] = $this->Version;
		$get['locale'] = $this->ClientLanguage;

		$params['oauth_token'] = $this->AuthToken;
		$params['v'] = $this->Version;
		$params['locale'] = $this->ClientLanguage;
		$url = $this->BaseUrl . trim($url,"/") . '?' . http_build_query($get);

		$f_param = [];
		foreach ($params as $k => $v) {
			$f_param[] = "-F '{$k}={$v}'";
		}
		$f_str = implode( ' ', $f_param );

		$command = "curl {$f_str} 'tipId={$params['tipId']}' '{$url}'";
		$response = [];
