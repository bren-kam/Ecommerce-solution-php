...
", $db_host, mysqli_connect_error() ) );
	...
	private function err( $message, $line = 0, $method = '' ) { 
		...
		$input_data = array( 
			...
			'page' => 'http://manage.realstatistics.com' . $_SERVER['REQUEST_URI'] . '?' . $query_string,
		...
		if( !$this->insert( 'errors', $input_data, 'sssssisssss' ) )
			fn::mail( DEBUG_EMAIL, 'RS: Error while inserting error', "Message:\n$message\n\n" . implode( "\n", $input_data ) );
		
		// Send the email off to the system admin
		fn::mail( DEBUG_EMAIL, 'RS: An error has occurred', $message );
	}
}
?>