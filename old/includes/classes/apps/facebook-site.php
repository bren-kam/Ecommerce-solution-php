...
class Facebook_Site extends Base_Class {
	...
	public function get_connected_website( $fb_page_id ) {
		...
		$website = $this->db->get_row( "SELECT a.`title`, c.`key` FROM `websites` AS a LEFT JOIN `sm_facebook_page` AS b ON ( a.`website_id` = b.`website_id` ) LEFT JOIN `sm_facebook_site` AS c ON ( b.`id` = c.`sm_facebook_page_id` ) WHERE b.`status` = 1 AND c.`fb_page_id` = $fb_page_id", ARRAY_A );
