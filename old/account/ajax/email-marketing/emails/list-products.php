...
foreach ( $products as $product ) {
	$dialog = '<a href="/dialogs/email-marketing/emails/get-website-product/?_nonce=' . $get_website_product_nonce . '&amp;pid=' . $product['product_id'] . '#dProductDialog' . $product['product_id'] . '" title="' . _('View Product') . '" rel="dialog">';
	$actions = '<a href="/ajax/email-marketing/emails/add-website-product/?_nonce=' . $add_website_product_nonce . '&amp;pid=' . $product['product_id'] . '" title="' . _('Add Product') . '" ajax="1">' . _('Add Product') . '</a>';
	$data[] = array( 
