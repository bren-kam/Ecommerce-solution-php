...
if ( isset( $_POST['_nonce'] ) && nonce::verify( $_POST['_nonce'], 'update-assigned-to' ) ) {
    ...
    $message .= 'http://admin.' . url::domain( $new_user['domain'], false ) . '/tickets/ticket/?tid=' . $_POST['tid'] . "\n\n";
    $message .= 'Priority: ' . $priorities[$tb['priority']] . "\n\n";
    $message .= "Sincerely,\n" . $new_user['company'] . " Team";

    fn::mail( $new_user['email'], 'You have been assigned Ticket #' . $_POST['tid'] . ' (' . $priorities[$tb['priority']] . ') - ' . $tb['summary'], $message, $new_user['company'] . ' <noreply@' . url::domain( $new_user['domain'], false ) . '>' );
