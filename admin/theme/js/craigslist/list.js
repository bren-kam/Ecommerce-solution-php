/**
 * Craigslist Templates List Page
 */
...
function postLoad( $ ) {
	...
	listCraigslist = $('#tListCraigslist').dataTable({
		'bProcessing': true,
		'bServerSide': true,
		'bAutoWidth': false,
		'sAjaxSource': '/ajax/craigslist/list/',
		't': $("#sAutoComplete").val(),
		'iDisplayLength' : 100,
		"oLanguage": {
...
function trSearchClick() {
	$.post( '/ajax/craigslist/search/', { '_nonce' : $('#_ajax_search').val(), 't' : $("#sAutoComplete").val(), 's' : $('#tAutoComplete').val() }, ajaxSearchClick, 'json' );
}
...
function trStateChange() {
	...
	$.post( '/ajax/craigslist/change-state/', { '_nonce' : $('#_ajax_change_state').val(), 's' : $(this).val() }, ajaxStateChange, 'json' );
}
...
function autocompleteSuccess( request, response ) {
	...
	$.post( '/ajax/craigslist/autocomplete/', { '_nonce' : $('#_ajax_autocomplete').val(), 'type' : cacheType, 'term' : request['term'] }, function( data ) {
