...
if ( isset( $_POST['_nonce'] ) && nonce::verify( $_POST['_nonce'], 'replace-product' ) ) {
	$errs = $v->validate();
	
	if ( empty( $errs ) ) {
		//$success = $p->replace_products( $_POST['hOldProductID'], $_POST['hNewProductID'] );
...
$new_product = $p->get( $p->get_new_ashley_product_id( $pid ) );

if ( $new_product ) {
	$new_product['images'] = $p->get_images( $pid );
	$new_product['categories_list'] = $p->get_categories( $pid );
	$new_product['tags'] = $ta->get( 'product', $pid );
	$new_product['attribute_items'] = $a->get_attribute_items_by_product( $pid );
}
...
<div id="content">
	...
	<table class="form">
		<tr>
			...
			<th width="500"><strong>New Product</strong></th>
		</tr>
		<tr>
			<td>Name:</td>
		...
		<tr>
			...
			<td>
				<div style="float:left">
					...
					<input type="hidden" name="hNewProductID" id="hOldProductID" value="<?php echo $product['product_id']; ?>" />
				...
				<div style="float:left; margin-left: 20px">
					<form name="fRemoveProduct" action="" method="post">
						...
						<input type="hidden" name="hProductID" id="hProductID" value="<?php echo $product['product_id']; ?>" />
