...
class Sweepstakes extends ActiveRecordBase {
    ...
    protected function get_account( $fb_page_id ) {
        return $this->prepare(
            'SELECT w.`website_id`, w.`domain`, sms.`email_list_id` FROM `websites` AS w LEFT JOIN `sm_facebook_page` AS smfbp ON ( smfbp.`website_id` = w.`website_id` ) LEFT JOIN `sm_sweepstakes` AS sms ON ( sms.`sm_facebook_page_id` = smfbp.`id` ) WHERE smfbp.`status` = 1 AND sms.`fb_page_id` = :fb_page_id'
            , 's'
            , array( ':fb_page_id' => $fb_page_id )
        )->get_row( PDO::FETCH_OBJ );
    }
