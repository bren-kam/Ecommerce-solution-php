...
class ApiExtLogReport extends CustomReport {
    ...
    public function report() {
        $report = $this->get_results( "SELECT w.`title`, ael.`api`, ael.`method`, ael.`url`, ael.`request`, ael.`raw_request`, ael.`response`, ael.`raw_response`, ael.`date_created` FROM `api_ext_log` AS ael LEFT JOIN `websites` AS w ON ( w.`website_id` = ael.`website_id` ) WHERE ael.`api` <> 'Google Maps API' AND ael.`date_created` > DATE_SUB( NOW(), INTERVAL 30 DAY ) ORDER BY ael.`id` DESC", PDO::FETCH_ASSOC );
