 ...
 * @since 1.0
 */
class AshleySpecificFeed extends ActiveRecordBase {
	...
	protected function get_website_product_skus( $account_id ) {
		return $this->prepare(
            'SELECT a.`product_id`, b.`sku` FROM `website_products` AS a LEFT JOIN `products` AS b ON ( a.`product_id` = b.`product_id` ) WHERE a.`website_id` = :account_id AND a.`active` = 1 AND b.`user_id_created` = :user_id_created', ARRAY_A );
	}
