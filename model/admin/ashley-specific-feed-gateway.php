...
class AshleySpecificFeedGateway extends ActiveRecordBase {
    ...
    protected $testing_sites = array( 78, 123, 124, 134, 158, 168, 175, 186, 190, 218, 228, 243, 291, 292, 293, 317, 318
        , 327, 335, 337, 354, 357, 377, 378, 403, 457, 458, 476, 477, 479, 527, 535, 559, 571, 573, 587, 590, 593, 596, 599, 600
	...
	public function run( Account $account, $file = '' ) {
        ...
        if ( empty( $file ) ) {
            ...
            $ticket->assigned_to_user_id = User::KERRY; //User::CHRIS;
    ...
    protected function get_existing_products() {
        $products = $this->prepare(
            ...
            WHERE p.`user_id_created` = :user_id_created AND p2.product_id IS NULL
            GROUP BY p.`sku`
            ORDER BY p.`publish_visibility` DESC"
            , 'i'
...
//            }
            ...
            $product->depth = $item['package-depth'];
            $product->height = $item['package-height'];
            $product->length = $item['package-length'];
    ...
    protected function get_product_info( $item ) {
        ...
        foreach ( $item->itemIdentification->packageCharacteristics as $ic ) {
            if ( $ic->packageDimensions->depth['unitOfMeasure'] == "Inches" ) {
                $new_product['package-depth'] = trim($ic->packageDimensions->depth['value']);
                $new_product['package-height'] = trim($ic->packageDimensions->height['value']);
                $new_product['package-length'] = trim($ic->packageDimensions->length['value']);
                break;
            }
        }
