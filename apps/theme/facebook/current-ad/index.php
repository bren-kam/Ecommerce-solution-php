...
if( $other_user_id == $user && $page_id ) {
	if( nonce::verify( $_POST['_nonce'], 'connect-to-field' ) ) {
		...
		if( empty( $errs ) )
			$success = $ca->connect( $page_id, $_POST['tFBConnectionKey'] );
	}
}

// Get the website
if( $page_id )
	$website = $ca->get_connected_website( $page_id );

add_footer('<div id="fb-root"></div>
<script>
  window.fbAsyncInit = function() {
    FB.init({appId: "186618394735117", status: true, cookie: true,
             xfbml: true});
...
<div id="content">
	<h1><?php echo _('Online Platform - Current Ad'); ?></h1>
	...
	?>
	<ol>
		<li>Go to this page: <a href="javascript:top.location.href='http://www.facebook.com/apps/application.php?id=186618394735117';" title="Online Platform - Current Ad">Online Platform - Current Ad</a> and click "Add to My Page" in the left-hand bar, select your page and add it.</li>
	...
	<?php } else { ?>
	<form name="fConnect" method="post" action="/facebook/current-ad/<?php if( isset( $_GET['app_data'] ) ) echo '?app_data=' . $_GET['app_data']; ?>">
