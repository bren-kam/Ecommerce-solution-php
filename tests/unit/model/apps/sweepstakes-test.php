...
class SweepstakesTest extends BaseDatabaseTest {
    ...
    public function testGetTabA() {
        ...
        // Insert About Us
        $this->phactory->insert( 'sm_sweepstakes', array( 'fb_page_id' => $fb_page_id, 'before' => $before ), 'is' );

        // Get it
        $tab = $this->sweepstakes->get_tab( $fb_page_id, $liked );

        $this->assertEquals( $tab->content, $before );

        // Delete it
        $this->phactory->delete( 'sm_sweepstakes', array( 'fb_page_id' => $fb_page_id ), 'i' );
    }
    ...
    public function testGetTabB() {
        ...
        // Insert About Us
        $this->phactory->insert( 'sm_sweepstakes', array( 'fb_page_id' => $fb_page_id, 'after' => $after ), 'is' );
    ...
    public function testGetConnectedWebsite() {
        ...
        $this->phactory->insert( 'sm_sweepstakes', array( 'sm_facebook_page_id' => $sm_facebook_page_id, 'fb_page_id' => $fb_page_id, 'key' => $key ), 'iis' );

        $account = $this->sweepstakes->get_connected_website( $fb_page_id );
    ...
    public function testConnect() {
        ...
        // Insert About Us
        $this->phactory->insert( 'sm_sweepstakes', array( 'key' => $key,  ), 's' );

        // Get it
        $this->sweepstakes->connect( $fb_page_id, $key );

        // Get the key
        $fetched_fb_page_id = $this->phactory->get_var( "SELECT `fb_page_id` FROM `sm_sweepstakes` WHERE `key` = '$key'" );

        $this->assertEquals( $fb_page_id, $fetched_fb_page_id );

        // Delete it
        $this->phactory->delete( 'sm_sweepstakes', array( 'key' => $key ), 'i' );
    }
