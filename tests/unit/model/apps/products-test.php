...
class ProductsTest extends BaseDatabaseTest {
    ...
    public function testGetTabA() {
        ...
        // Insert About Us
        $this->db->insert( 'websites', array( 'website_id' => $account_id, 'title' => 'Banagrams', 'product_catalog' => $product_catalog ), 'isi' );
        $this->db->insert( 'sm_products', array( 'sm_facebook_page_id' => $sm_facebook_page_id, 'fb_page_id' => $fb_page_id, 'content' => $content ), 'iis' );
        $this->db->insert( 'sm_facebook_page', array( 'id' => $sm_facebook_page_id, 'website_id' => $account_id ), 'ii' );
        ...
        $this->db->delete( 'sm_products', array( 'fb_page_id' => $fb_page_id ), 'i' );
    ...
    public function testGetConnectedWebsite() {
        ...
        $this->db->insert( 'sm_facebook_page', array( 'id' => $sm_facebook_page_id, 'website_id' => $account_id ), 'iii' );
        $this->db->insert( 'sm_products', array( 'sm_facebook_page_id' => $sm_facebook_page_id, 'fb_page_id' => $fb_page_id, 'key' => $key ), 'iis' );
    ...
    public function testConnect() {
        ...
        // Insert About Us
        $this->db->insert( 'sm_products', array( 'key' => $key,  ), 's' );

        // Get it
        $this->products->connect( $fb_page_id, $key );

        // Get the key
        $fetched_fb_page_id = $this->db->get_var( "SELECT `fb_page_id` FROM `sm_products` WHERE `key` = '$key'" );

        $this->assertEquals( $fb_page_id, $fetched_fb_page_id );

        // Delete it
        $this->db->delete( 'sm_products', array( 'key' => $key ), 'i' );
    }
