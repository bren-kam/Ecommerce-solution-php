...
class Compress_CSS {
	...
	private function parse( $contents ) {
			...
			while( $declaration_key < count( $declarations ) ) {
				...
				if( !empty( $property ) || !empty( $value ) ) {
					...
					foreach( $selectors as $s ) {
						...
						if( !is_array( $css_array[$key] ) || !in_array( $s, $css_array[$key] ) ) {
							// Makes sure there are no duplicate properties for selectors
							if( is_array( $this->selectors[$s] ) ) {
		...
		foreach( $css_array as $declaration => $selectors ) {
			...
			if( !is_array( $selector_array[$selector_key] ) || !in_array( $new_declaration, $selector_array[$selector_key] ) )
