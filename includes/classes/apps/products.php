		...
		if( 'no-catalog' == $tab_data['content'] ) {
			...
			foreach( $categories as $cat ) {
				...
				if( 0 == $image_width || 0 == $image_height ) {
					...
					$this->db->insert( 'website_image_dimensions', array( 'website_id' => $tab_data['website_id'], 'image_url' => $cat['image_url'], 'width' => $image_width, 'height' => $image_height, 'date_created' => date_time::date('Y-m-d H:i:s') ), 'isiis' );
