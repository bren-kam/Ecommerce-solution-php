...
class Tags extends Base_Class {
	 ...
	 * @return bool
	 */
	public function autocomplete( $type, $query ) {
		// Get results
		$results = $this->db->prepare( "SELECT DISTINCT( `value` ) FROM `tags` WHERE `type` = ? AND `value` LIKE ? ORDER BY `value`", 'ss', $type, $query . '%' )->get_results( '', ARRAY_A );
