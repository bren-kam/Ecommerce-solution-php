...
class Ticket_Comments extends Base_Class {
	...
	public function add( $ticket_id, $user_id, $comment, $private = 0, $attachments = array() ) {
		$this->db->insert( 'ticket_comments', array( 'ticket_id' => $ticket_id, 'user_id' => $user_id, 'comment' => $comment, 'private' => $private, 'date_created' => date_time::now() ), 'iisis' );
		
		// If there was an error
		if( $this->db->errno() ) {
			$this->err( 'Failed to add ticket comment.', __LINE__, __METHOD__ );
			return false;
		}
	...
	public function link_attachments( $ticket_comment_id, $attachments ) {
		...
		// If there was an error
		if( $this->db->errno() ) {
			$this->err( 'Failed to link ticket comments to upload links.', __LINE__, __METHOD__ );
			return false;
		}
	...
	public function update( $ticket_comment_id, $comment ) {
		...
		// If there was an error
		if( $this->db->errno() ) {
			$this->err( 'Failed to update ticket comment.', __LINE__, __METHOD__ );
			return false;
		}
	...
	public function get( $ticket_id ) {
		...
		// If there was an error
		if( $this->db->errno() ) {
			$this->err( "Failed to get ticket comments.", __LINE__, __METHOD__ );
			return false;
		}
	...
	public function get_single( $ticket_comment_id ) {
		...
		// If there was an error
		if( $this->db->errno() ) {
			$this->err( "Failed to get a ticket comment.", __LINE__, __METHOD__ );
			return false;
		}
	...
	public function delete( $ticket_comment_id ) {
		...
		// If there was an error
		if( $this->db->errno() ) {
			$this->err( "Failed to delete ticket comment.", __LINE__, __METHOD__ );
			return false;
		}
		...
		if( is_array( $attachments ) ) {
			...
			// If there was an error
			if( $this->db->errno() ) {
				$this->err( "Failed to delete ticket comment upload links.", __LINE__, __METHOD__ );
				return false;
			}
		}
