...
class Websites extends Base_Class {
	 ...
	 * @return int|bool
	 */
    public function create( $user_id, $os_user_id, $domain, $title, $type ) {
		$this->db->insert( 'websites', array( 
			...
			'title' => $title, 
			'type' => $type,
			'date_created' => date_time::now() ), 'iissss');
	...
	public function get_website( $website_id ) {
		$website = $this->db->get_row( 'SELECT `website_id`, `os_user_id`, `user_id`, `domain`, `subdomain`, `title`, `theme`, `logo`, `phone`, `products`, `product_catalog`, `link_brands`, `blog`, `email_marketing`, `shopping_cart`, `seo`, `room_planner`, `domain_registration`, `additional_email_addresses`, `ga_profile_id`, `ga_tracking_key`, `wordpress_username`, `wordpress_password`, `mc_list_id`, `type`, `version`, `live`, `date_created`, `date_updated`  FROM `websites` WHERE `website_id` = ' . (int) $website_id, ARRAY_A );
	...
	public function list_websites( $where, $order_by, $limit ) {
		...
		// $websites = $this->db->get_results( "SELECT a.`website_id`, a.`domain`, a.`title`, a.`products`, b.`user_id`, b.`company_id`, b.`contact_name`, b.`store_name` FROM `websites` as a INNER JOIN `users` as b ON ( a.`user_id` = b.`user_id` ) LEFT JOIN `website_products` AS c ON ( a.`website_id` = c.`website_id` ) $where GROUP BY a.`website_id` ORDER BY $order_by LIMIT $limit", ARRAY_A );
		$websites = $this->db->get_results( "SELECT a.`website_id` AS website_id, a.`domain`, a.`title`, a.`products`, b.`user_id`, b.`company_id`, b.`contact_name`, b.`store_name`, ( SELECT COUNT( DISTINCT z.`product_id` ) FROM `website_products` AS z LEFT JOIN `products` AS y ON ( z.`product_id` = y.`product_id` ) WHERE z.`active` = 1 AND y.`publish_visibility` <> 'deleted' AND z.`website_id` = a.`website_id` ) AS used_products FROM `websites` as a INNER JOIN `users` as b ON ( a.`user_id` = b.`user_id` ) LEFT JOIN `website_products` AS c ON ( a.`website_id` = c.`website_id` ) $where GROUP BY a.`website_id` ORDER BY $order_by LIMIT $limit", ARRAY_A );
		// @Fix - this has an inline query and is ever so slightly on the slow side.  Update in future maybe ^
...
0 23 * * 0 /usr/bin/php /home/' . $username . '/public_html/' . $subdomain . 'core/crons/weekly.php > /dev/null" | crontab' );
	...
	public function delete( $website_id ) {
		...
		// @Fix in PHP 3.0 -- can typecast variables
		// Typecast
		$website_id = (int) $website_id;
		
		// Has to be Kerry or Tom
		if( !in_array( $user['user_id'], array( 1, 335 ) ) )
			return false;
