 ...
 * @since 1.0
 */
class Products2 extends Base_Class {
	/**
	...
	public function update( $name, $slug, $description, $status, $sku, $price, $list_price, $product_specifications, $brand_id, $industry_id, $publish_visibility, $publish_date, $product_id, $weight = 0, $volume = 0 ) {
		...
		$this->db->update( 'products2', array(
				'brand_id' => $brand_id,
	...
	public function add_product_images( $images, $product_id ) {
		...
		$this->db->query( "INSERT INTO `product_images2` ( `product_id`, `image`, `sequence` ) VALUES $values" );
	...
	private function add_product_category( $product_id, $category_id, $website_ids ) {
		...
		$image = $this->db->get_var( "SELECT `image` FROM `product_images2` WHERE `product_id` = $product_id AND `sequence` = 0 LIMIT 1" );
	...
	public function empty_product_images( $product_id ) {
		$this->db->query( 'DELETE FROM `product_images2` WHERE `product_id` = ' . (int) $product_id );
	...
	public function list_products( $where, $order_by, $limit ) {
		$products = $this->db->get_results( "SELECT a.`product_id`, a.`name`, d.`name` AS brand, a.`sku`, a.`status`, DATE( a.`publish_date` ) AS publish_date, c.`name` AS category, e.`image`, e.`swatch` FROM  AS a LEFT JOIN `product_categories` AS b ON (a.product_id = b.product_id) LEFT JOIN `categories` AS c ON (b.category_id = c.category_id) LEFT JOIN `brands` AS d ON (a.brand_id = d.brand_id) LEFT JOIN `product_images2` AS e ON (a.`product_id` = e.`product_id`) WHERE ( e.`sequence` = 0 OR e.`sequence` IS NULL ) $where GROUP BY a.`product_id` ORDER BY $order_by LIMIT $limit", ARRAY_A );
	...
	public function count_products( $where ) {
		...
		$product_count = $this->db->get_col( "SELECT a.`product_id` FROM  AS a LEFT JOIN `product_categories` AS b ON (a.product_id = b.product_id) LEFT JOIN `categories` AS c ON (b.category_id = c.category_id) LEFT JOIN `brands` AS d ON (a.brand_id = d.brand_id) LEFT JOIN `product_images2` AS e ON (a.`product_id` = e.`product_id`) WHERE ( e.`sequence` = 0 OR e.`sequence` IS NULL ) $where GROUP BY a.`product_id`" );
	...
	public function autocomplete( $query, $field, $as = '', $where = '' ) {
		...
		// Get results
		$results = $this->db->prepare( "SELECT DISTINCT( $field )$as FROM  AS a LEFT JOIN `product_categories` AS b ON (a.product_id = b.product_id) LEFT JOIN `categories` AS c ON (b.category_id = c.category_id) LEFT JOIN `brands` AS d ON (a.brand_id = d.brand_id) LEFT JOIN `product_images2` AS e ON (a.`product_id` = e.`product_id`) WHERE e.`sequence` = 0 AND $field LIKE ? $where GROUP BY a.`product_id` ORDER BY $field", 's', $query . '%' )->get_results( '', ARRAY_A );
	...
	public function clone_product( $product_id ) {
		...
		$exists = $this->db->get_var( "SELECT `product_id` FROM  WHERE `product_id` = $product_id" );
		...
		// Clone product
		$this->db->query( "INSERT INTO  ( `brand_id`, `industry_id`, `name`, `slug`, `description`, `status`, `sku`, `price`, `list_price`, `product_specifications`, `publish_visibility`, `publish_date`, `user_id_created`, `date_created` ) SELECT `brand_id`, `industry_id`, CONCAT( `name`, ' (Clone)' ), CONCAT( `slug`, '-2' ), `description`, `status`, CONCAT( `sku`, '-2' ), `price`, `list_price`, `product_specifications`, `publish_visibility`, `publish_date`, $user_id, NOW() FROM  WHERE `product_id` = $product_id" );
	...
	public function remove_image( $image, $product_id ) {
		$this->db->prepare( 'DELETE FROM `product_images2` WHERE `image` = ? AND `product_id` = ?', 'si', $image, $product_id )->query('');
