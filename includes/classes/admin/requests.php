...
class Requests extends Base_Class {
		...
		switch( $request['type'] ) {
			case 'Header Update':
				...
				if( isset( $request_data['image'] ) && $ftp = new FTP( $request['website_id'] ) ) {
					...
					$local_file_path = '/home/imaginer/public_html/account' . $url_data['path'];
	...
	public function send_message( $request_id, $message ) {
		...
		$this->db->insert( 'request_messages', array( 'request_id' => $request_id, 'user_id' => $user['user_id'], 'message' => $message, 'date_created' => date_time::date('Y-m-d H:i:s') ), 'iiss' );
