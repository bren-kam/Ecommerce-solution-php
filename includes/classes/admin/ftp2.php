...
/**
 * Handles all FTP connections with clients websites
 */
class Ftp_Model extends Model {
	...
	public function __construct( $website_id, $cwd = '', $skip = false ) {
		...
		if( !$skip ) {
			...
			$this->username = security::decrypt( base64_decode( $ftp_data['ftp_username'] ), Kohana::config('keys.ENCRYPTION_KEY') );
			$this->password = security::decrypt( base64_decode( $ftp_data['ftp_password'] ), Kohana::config('keys.ENCRYPTION_KEY') );
			$this->port     = 21;
	 ...
	 *
	 * @var string $local_file_path a path to the local file to be moved
	 * @var string $remote_file_path a path to the destination on remote server
	 * @var constant (optional) FTP_ASCII/FTP_BINARY transfer method
	 * @return bool
	 */
	 ...
	 *
	 * @var string $local_file_path a path to the local file to be saved
	 * @var string $remote_file_path a path to the destination on remote server
	 * @var constant (optional) FTP_ASCII/FTP_BINARY transfer method
	 * @return bool
	 */
	 ...
	 *
	 * @var octal $permissions the permissions to set the directory, must be in octal
	 * @var string $remote_file_path the path to the file you want to change
	 * @return bool
	 */
	 ...
	 *
	 * @var string $remote_directory the remote directory to change to
	 * @return bool
	 */
	 ...
	 * Make a directory
	 *
	 * @var string $remote_directory the remote directory to create
	 * @return bool
	 */
	 ...
	 * Remove a directory
	 *
	 * @var string $remote_directory the remote directory to remove
	 * @return bool
	 */
	 ...
	 *
	 * @var string $old_name the name of the file/directory
	 * @var string $new_name the new name of the file/directory
	 * @return bool
	 */
	 ...
	 *
	 * @var string $remote_directory the directory to list contents of
	 ...
	 * turns it into octal.
	 *
	 * @var reference $i the number to check
	 */
