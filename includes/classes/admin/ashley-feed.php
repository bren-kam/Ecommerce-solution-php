...
class Ashley_Feed extends Base_Class {
	...
	public function run( $website_id, $file = '' ) {
		...
		$ftp->username = security::decrypt( base64_decode( $settings['username'] ), ENCRYPTION_KEY );
		$ftp->password = security::decrypt( base64_decode( $settings['password'] ), ENCRYPTION_KEY );
		$ftp->port     = 21;
	...
	private function get_website_product_skus( $website_id ) {
		...
		// Get Products
		$products = $this->db->get_results( "SELECT a.`product_id`, b.`sku` FROM `website_products` AS a LEFT JOIN `products` AS b ON ( a.`product_id` = b.`product_id` ) WHERE a.`website_id` = $website_id AND a.`active` = 1", ARRAY_A );
