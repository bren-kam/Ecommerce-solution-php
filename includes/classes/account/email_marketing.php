...
class Email_Marketing extends Base_Class {
	...
	public function unsubscribe( $email_id, $email ){
		...
		// First, get mailchimp and unsubscribe the email - that way, emails that fail to unsub aren't deleted from our DB
	...
	public function create_email( $email, $name, $phone ) {
		...
		$this->db->insert( 'emails', array( 'website_id' => $user['website']['website_id'], 'name' => $name, 'email' => $email, 'phone' => $phone, 'status' => 1, 'date_created' => date_time::date('Y-m-d H:i:s') ), 'isssis' );
	...
	public function schedule_email( $email_message_id ) {
		...
		if ( strtotime( $em['date_sent'] ) > time() ) {
			$mc->campaignSchedule( $em['mc_campaign_id'], date_time::date( 'Y-m-d H:i:s', strtotime( $em['date_sent'] ) + 18000 ) );
	...
	public function create_email_list( $name, $description ) {
		...
		$this->db->insert( 'email_lists', array( 'name' => $name, 'description' => $description, 'website_id' => $user['website']['website_id'], 'date_created' => date_time::date('Y-m-d H:i:s') ), 'ssis' );
	...
	public function add_email_message( $email_template_id, $subject, $message, $type, $date_sent, $email_list_ids, $message_meta ) {
		...
		$this->db->insert( 'email_messages', array( 'website_id' => $user['website']['website_id'], 'email_template_id' => $email_template_id, 'subject' => $subject, 'message' => $message, 'type' => $type, 'date_sent' => $date_sent, 'date_created' => date_time::date('Y-m-d H:i:s') ), 'iisssss' );
	...
	public function create_autoresponder( $name, $subject, $message, $current_offer, $email_list_id ) {
		...
		$this->db->insert( 'email_autoresponders', array( 'website_id' => $user['website']['website_id'], 'email_list_id' => $email_list_id, 'name' => $name, 'subject' => $subject, 'message' => $message, 'current_offer' => $current_offer, 'date_created' => date_time::date('Y-m-d H:i:s') ), 'iisssis' );
	...
	private function remove_bad_emails() {
		...
		$unsubscribers = $mc->listMembers( $user['website']['mc_list_id'], 'unsubscribed', date_time::date( 'Y-m-d H:i:s', time() - 86700 ), 0, 15000 );
		...
		// Get the cleaned
		$cleaned = $this->mc->listMembers( $user['website']['mc_list_id'], 'cleaned', date_time::date( 'Y-m-d H:i:s', time() - 86700 ), 0, 10000 );
	...
	public function transfer_emails( $site_a, $site_b ) {	
		$emails_a = $this->db->prepare( 'SELECT * FROM `emails` WHERE `website_id` = ?', 'i', $site_a )->get_results( '', ARRAY_A );
		$emails_b = $this->db->prepare( 'SELECT * FROM `emails` WHERE `website_id` = ?', 'i', $site_b )->get_results( '', ARRAY_A );
		
		$email_lists_a = $this->db->prepare( 'SELECT * FROM `email_lists` WHERE `website_id` = ?', 'i', $site_a )->get_results( '', ARRAY_A );
		$email_lists_b = $this->db->prepare( 'SELECT * FROM `email_lists` WHERE `website_id` = ?', 'i', $site_b )->get_results( '', ARRAY_A );
		
		foreach( $emails_a as $ea ) {
			$email_associations = $this->db->prepare( 'SELECT * FROM `email_associations` WHERE `email_id` = ?', 'i', $ea['email_id'] )->get_results( '', ARRAY_A );
			
			foreach( $email_associations as $a ) {	
				foreach( $emails_b as $eb ) {
					if( ( $eb['email'] == $ea['email'] ) && ( $eb['date_created'] == $ea['date_created'] ) ) {
						$new_email_id = $eb['email_id'];
						break;
					}
				}
				
				// Fetch the old email list properties
				foreach( $email_lists_a as $ela ) {
					if( $ela['email_list_id'] == $a['email_list_id'] ) {
						$old_email_list = $ela;
						break;
					}
				}
				
				foreach( $email_lists_b as $elb ) {
					if( ( $elb['category_id'] == $old_email_list['category_id'] ) && ( $elb['name'] == $old_email_list['name'] ) ) {
						$new_email_list_id = $elb['email_list_id'];
						break;
					}
				}
				
				$insert_sql[] = '(' . $new_email_id . ',' . $new_email_list_id . ')';				
			}
		}
		
		$this->db->query( 'INSERT INTO `email_associations` ( `email_id`, `email_list_id` ) VALUES ' . implode( ',', $insert_sql ) );
		
		// Handle any error
		if ( $this->db->errno() ) {
			$this->err( 'Failed to mass-produce email associations.', __LINE__, __METHOD__ );
			return false;
		}
	}
